...
from api_call import key_infos, verify_key
from config.utilities import format_timestamp

def key_infos_tab(app):
        print("\nKey infos tab opened!")

        key_frame = ctk.CTkFrame(app.main_frame)
        key_title = ctk.CTkLabel(key_frame, text="Please enter your Key to fetch its Infos:", font=app.main_font)
        app.key_input = ctk.CTkEntry(key_frame, placeholder_text="Put your Key here", font=app.main_font, width=450)
        activate_button = ctk.CTkButton(app.main_frame, text="Fetch the Key infos!", command=lambda: on_infos(app), font=app.main_font)
...
def on_infos(app):
    ...
    if verify_result.get("valid") == True and data != None:
        license_id = data["id"]
        result = key_infos(license_id)
        if result.get("success"):
            key_data = result["key_data"]
            attributes = key_data["data"]["attributes"]
            key = attributes["key"]
            name = attributes["name"]
            expiry = attributes.get("expiry", "LIFETIME")
            created = attributes["created"]
            status = attributes["status"]
            machine_udid = attributes["machine_udid"]

            app.success_msg = ctk.CTkLabel(app.message_frame, text=f"Key `{license_key}` infos are fetched!", text_color="green", font=app.log_font)
            app.success_msg.pack(padx=10, pady=10)
            app.info_msg = ctk.CTkLabel(app.message_frame, text=f"Name: {name}\nKey: {key}\nCreated: {format_timestamp(created)}\nExpiry: {format_timestamp(expiry) if expiry != None else "LIFETIME"}\nMachine UDID: {machine_udid}\nStatus: {status}", font=app.log_font)
            app.info_msg.pack(padx=10, pady=10, fill="both")
            app.key_input.delete(first_index=0, last_index=len_k)
            print("Successfuly fetched the key infos!")
        ...
        print(f"Couln't fetch the key infos! (unvalid key)")
    else:
        error = result.get("error")
        app.fail_msg = ctk.CTkLabel(app.message_frame, text=f"Key `{license_key}` infos couln't be fetched!\nError: {error}", text_color="red", font=app.log_font)
        app.fail_msg.pack(padx=10, pady=10)
        print(f"Couln't fetch the key infos! ({error})")